...
# Global states initialized for trap safety
export PIP_UNINSTALL_PSSH="no"
export CLEANUP_DONE="no"
export PSSHPID=""
export NCPID=""
export RETVAL=0
...
function setup_host_env() {
	...
	install-depends time openssh-clients
	install_numad
	install_tuned
...
function teardown() {
	...
	teststate_log "test end :: $(date +%s)" || true
...
function main() {
	...
	synchronize_vms
	collect_results
	# teardown is executed automatically
}
