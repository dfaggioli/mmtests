...
# In any case, we will always check at least in MMTests main directory.
...
if [ "$MMTESTS_VMS_IP" != "" ]; then
	...
	# All VMs should be up and running now, so this is a good time for doing
	...
	#
	# In fact, it is ok if these scripts reboot one or more VMs. What's
